    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn style="margin-left: 5px;" @click="requestAuthorRegistrationDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('Author')">
                ...
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="updateAuthorProfileDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('Author')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>작가 정보 수정
                </v-btn>
                <v-dialog v-model="updateAuthorProfileDialog" width="500">
                    <UpdateAuthorProfile
                        @closeDialog="updateAuthorProfileDialog = false"
            ...
            <AuthorStatistics @search="search" style="margin-bottom: 10px; background-color: #ffffff;"></AuthorStatistics>
...
export default {
    ...
    data: () => ({
        ...
        updateAuthorProfileDialog: false,
    ...
    methods:{
        ...
        async updateAuthorProfile(params){
            try{
                var path = "updateAuthorProfile".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, params)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','UpdateAuthorProfile 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
                }
                this.updateAuthorProfileDialog = false
            }catch(e){
                console.log(e)
            }
        },
